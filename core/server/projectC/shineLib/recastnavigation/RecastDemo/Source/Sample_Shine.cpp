...
const float ppp[] = {
7.020000,0.043040,30.910000,
23.800001,6.443040,30.800001,
25.600000,0.043040,30.800001,
30.219999,0.043040,30.379999
};
...
int Sample_Shine::rasterizeTileLayers(
...
void Sample_Shine::handleSettings()
{
	...
	imguiLabel("Tiling");
	imguiSlider("TileSize", &m_tileSize, 16.0f, 256.0f, 8.0f);
	...
	if (imguiButton("Save"))
	{
		saveAll((DirFront + "tiles.bin").c_str());
	}
	...
	if (imguiButton("Load"))
	{
		dtFreeNavMesh(m_navMesh);
		dtFreeTileCache(m_tileCache);
		loadAll((DirFront + "tiles.bin").c_str());
...
void Sample_Shine::handleDebugMode()
{
...
void Sample_Shine::handleRender()
{
	...
	{
		duDebugDrawCircle(&m_dd, ppp[i * 3], ppp[i * 3 + 1], ppp[i * 3 + 2], 0.3f, duRGBA(0, 0, 0, 128), 0.3f);
	}
...
void Sample_Shine::renderCachedTile(const int tx, const int ty, const int type)
{
	if (m_tileCache)
		drawDetail(&m_dd, m_tileCache, tx, ty, type);
}

void Sample_Shine::renderCachedTileOverlay(const int tx, const int ty, double* proj, double* model, int* view)
{
	if (m_tileCache)
		drawDetailOverlay(m_tileCache, tx, ty, proj, model, view);
}

void Sample_Shine::handleRenderOverlay(double* proj, double* model, int* view)
{
	if (m_tool)
...
void Sample_Shine::handleMeshChanged(class InputGeom* geom)
{
...
void Sample_Shine::addTempObstacle(const float* pos)
{
	if (!m_tileCache)
		return;
	float p[3];
...
void Sample_Shine::removeTempObstacle(const float* sp, const float* sq)
{
	if (!m_tileCache)
		return;
...
void Sample_Shine::clearAllTempObstacles()
{
	if (!m_tileCache)
		return;
...
void Sample_Shine::handleUpdate(const float dt)
{
...
void Sample_Shine::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void Sample_Shine::saveAll(const char* path)
{
...
void Sample_Shine::loadAll(const char* path)
{
	FILE* fp = fopen(path, "rb");
	if (!fp) return;

	int a1 = sizeof(TileCacheSetHeader);
	...
	int omlLen;
	fread(&omlLen, sizeof(int), 1, fp);

	for (int i = 0; i < omlLen; ++i)
	{
		AutoOffMeshLinkData v;
		fread(&v, sizeof(AutoOffMeshLinkData), 1, fp);
...
void Sample_Shine::makeOML()
{
	offMeshConCount = (int)m_OffMeshLinks.size();
