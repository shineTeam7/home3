 ...
 * specify something in one of the two fields, but not the social.
 * <P>
...
public final class CronExpression implements Serializable, Cloneable
{
	...
	{
		/*
		 * We don't call the social constructor here since we need to swallow the
	...
	protected void buildExpression(String expression) throws ParseException
	{
		...
		try
		{
			...
			while(exprsTok.hasMoreTokens() && exprOn<=YEAR)
			{
				...
				// throw an exception if L is used with social days of the month
				if(exprOn==DAY_OF_MONTH && expr.indexOf('L')!=-1 && expr.length()>1 && expr.contains(","))
				{
					throw new ParseException("Support for specifying 'L' and 'LW' with social days of the month is not implemented",-1);
				}
				// throw an exception if L is used with social days of the week
				if(exprOn==DAY_OF_WEEK && expr.indexOf('L')!=-1 && expr.length()>1 && expr.contains(","))
				{
					throw new ParseException("Support for specifying 'L' with social days of the week is not implemented",-1);
				}
			...
			// Copying the unit from the UnsupportedOperationException below
