...
///
/// </summary>
public class PlayerEquipContainerTool:PlayerFuncTool
{
	/** 数据 */
	...
	public PlayerEquipContainerTool(int funcID):base(FuncToolType.EquipContainer,funcID)
	{
	...
	public override void afterReadDataSecond()
	{
		...
		{
			foreach(var kv in fDic.entrySet())
			{
				kv.value.makeConfig();
				addEquipInfluence(kv.key,kv.value);
			}
		}
	}
	...
	{
		return GameC.factory.createEquipContainerData();
	}
