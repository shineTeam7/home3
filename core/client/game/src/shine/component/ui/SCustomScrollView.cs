...
namespace ShineEngine
{
	...
	public class SCustomScrollView:GridContainer, IPointerDownHandler, IPointerUpHandler, IDragHandler, IBeginDragHandler
	{
		...
		public void setOnbackTween(bool miss)
		{
			missOnback = miss;
		}
		...
		{
			if(missOnback)return;
			setScrollPos(value);
		}

		private void onFrame(int interval)
		{
			if(Math.Abs(_touchSpeed) <= SpeedEnd || (!_loop && (_scrollPos > _maxRealPos + _maxOutDistance || _scrollPos < _minRealPos - _maxOutDistance)))
			{
				stopSpeed();
				checkAndBack();
				return;
